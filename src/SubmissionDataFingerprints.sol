...
contract SubmissionDataFingerprints is Ownable {
     ...
     */
    function getUserRecordBySubmissionId(address user, uint256 submissionId, uint256 page, uint256 size)
        public
        view
        returns (bool found, Record memory record, bool end)
    {
     ...
     *     @return end if to the end
     */
    function getUserRecords(address user, uint256 page, uint256 size)
        public
        view
        returns (Record[] memory rets, bool end)
    {
        Record[] storage records = storedRecords[user];
        if (records.length == 0) {
